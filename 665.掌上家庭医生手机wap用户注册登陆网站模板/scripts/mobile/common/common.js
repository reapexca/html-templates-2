//(!!window.Hammer) || document.write('<script src="../img.familydoctor.com.cn/scripts/mobile/common/hammer.min.js"><\/script>');

if(!window.Hammer){
    $.holdReady(true);
    $.getScript("../img.familydoctor.com.cn/scripts/mobile/common/hammer.min.js", function(){
        $.holdReady(false);
    });
}
...
$(document).ready(function(){
    ...
    (function(){
        ...
        var $item = $wrap.find("a");
        $wrap.find(".more")
            .css({left: $item.eq(4).offset().left})
            .click(function(){
                $(this).toggleClass("more-up");
                $wrap.toggleClass("slide-down");
            });
    })();
    ...
    (function(){
        var $rdos = $(".radio-box");
        if($rdos.length === 0){return false;}

        $rdos.each(function(){
            var $rdo = $(this).find("input[type='radio']");
            $rdo.wrap('<div class="invisible"></div>');
            var radioName = $rdo.attr("name");

            if($rdo.attr("checked") === "checked"){
                $(this).prepend('<em class="'+ radioName + "Box" +'"></em>');
            }else{
                $(this).prepend('<em class="'+ radioName + "Box off" +'"></em>');
            }

            var t1 = null;
            var $ems = $("." + radioName + "Box");
            $ems.each(function(index){
                $(this).click(function(){
                    //防止uc浏览器触发两次click
                    if (t1 == null){
                        t1 = new Date().getTime();
                    ...
                    var $curRadios = $("input[name='"+ radioName +"']");
                    $curRadios.removeAttr("checked").eq(index).attr("checked", "checked");
                });
            });
        });
    })();
    ...
    (function(){
        ...
        $tabs.each(function(index){
            ...
            $(this).click(function(){
                ...
                $tabs.removeClass("on");
                $(this).addClass("on");
                $items.addClass("hide").eq(index).removeClass("hide");
                myScroll.refresh();
            });
        });
	...
	//读取已登录的会员名
	(function(){
		var userName = getCookies("dnt", "username");
		if(!!userName){
			$("#login-user-name").html('<a href="ask/PersonalCenter/InfoCenter/default.htm">' + userName + '</a>');
		}
	})();
});
...
function getCookieBySubName(sMainName, sSubName) {
...
//模拟下拉列表(调用方式：$(".ddl").mobileDDL())
(function($){
    $.fn.mobileDDL = function(){
        $("body").append('<div class="ddl-mask" id="ddl-mask"></div><div class="ddl-wrap" id="ddl-wrap"></div>');
        var $mask = $("#ddl-mask");
        var $wrap = $("#ddl-wrap");
        ...
        return this.each(function(){
            var $that = $(this);
            var $list = $that.find(".ddl-list");
            var $items = $list.find("li");
            var $trigger = $that.find(".ddl-val");

            $trigger.click(function(){
                var $me = $(this);
                $list.wrap('<div class="ddl-inner"></div>').show().parent().appendTo($wrap);
                $mask.show();
                $wrap.show();

                $items.each(function(){
                    if($(this).attr("data-val") === $me.attr("data-val")){
                        $(this).addClass("on");
                    }else{
                        $(this).removeClass("on");
                    }
            ...
            $items.on("click", function(){
                $trigger.attr("data-val", $(this).attr("data-val"));
                $trigger.html($(this).html()).css({height: $trigger.parent().height()});
                $mask.hide();
                $wrap.hide();
            });
...
(function($){
    $.alert = function(settings){
        var config = {
            ...
            'cancelText': "取消",
            'showClose': true,
            'showConfirm': true, //默认显示"确定"按钮
            'showCancel': false, //默认不显示"取消"按钮
            'flag': false,
            'callback': function(){}
        };
        ...
        $("#alert-mask").remove();
        $("#alert-win").remove();
        $("body").append('<div id="alert-mask" class="alert-mask"></div><div id="alert-win" class="alert-win"><div class="alert-inner"><div class="alert-sbj"><b>'+ config.title +'</b></div><a href="javascript:;" class="close">&times;</a><p>'+ config.msg +'</p><div class="alert-btn"><input type="button" value="'+ config.confirmText +'" class="confirm" /><input type="button" value="'+ config.cancelText +'" class="cancel" /></div></div></div>');
        var $win = $("#alert-win");
        var $mask = $("#alert-mask");
        $mask.css({height: $(document).height()}).show();
        $win.css({top: ($(window).height() - $win.outerHeight())/2}).fadeIn();

        var $btnWrap = $win.find(".alert-btn");
        var $confirm = $btnWrap.find(".confirm");
        var $cancel = $btnWrap.find(".cancel");
        var $close = $win.find(".close");
    ...
    $.fn.calendar = function(settings){
        ...
        function init($input){
            ...
            if(config.defaultDate.constructor == String){
                var str = config.defaultDate.replace(/-/g, "default.htm");
        ...
        function setSelectedDate($wrap){
            var str = "";
            if(config.selectedDate.constructor != String || isNaN(Date.parse(str = config.selectedDate.replace(/-/g, "default.htm")))){
...
//百度分享
(function($){
    $.fn.baiduShare = function(settings){
        var config = {
            top: 50
        };
        if (settings){$.extend(config, settings);}
        ...
        return this.each(function(){
            ...
            $(this).hammer().on("tap", function(e){
                ...
                if($shareBox.length === 0){
                    $("body").append('<div class="share-box" id="share-box"></div>');
                    $shareBox = $("#share-box");
                    var str = '<div class="bdsharebuttonbox"><ol><li><a href="#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a></li><li><a href="#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a></li><li><a href="#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a></li><li><a href="#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a></li></ol></div><ul><li>微信</li><li>QQ空间</li><li>新浪微博</li><li>腾讯微博</li></ul>';
                    ...
                    $shareBox.html(str);
                    $shareBox.append('<i></i><em></em>');
                    $shareBox.find("em").css({left: pos.left + ($that.outerWidth()/2) - 16 + "px"});
                    $shareBox.find("i").css({left: pos.left + ($that.outerWidth()/2) - 17 + "px"});
                }

                $that.toggleClass("on");
                $shareBox.css({top: pos.top + config.top + "px"}).toggle();
                e.stopPropagation();
                ...
                $shareBox.hammer().on("tap", function(e){
                    e.stopPropagation();
                });
            });
        });
...
//图片浏览(弹出层)
(function($){
	$.fn.photoView = function(settings){
		config = {}
		if(settings){$.extend(config, settings);}
		...
		return this.each(function(){
			...
			$photos.click(function(){
				$("body").append('<div class="dd-photo-mask" id="dd-photo-mask"></div><div class="dd-photo-wrap" id="dd-photo-wrap"></div>');
				var arr = [], nav = [];
				var len = $photos.length;
				var $mask = $("#dd-photo-mask");
				var $wrap = $("#dd-photo-wrap");
				
				$photos.each(function(){
					arr.push('<li><img src="'+ $(this).attr("data-url") +'" /></li>');
					nav.push('<a href="javascript:;"></a>');
				});

				$wrap.html('<ul>' + arr.join("") + '</ul><div class="dd-nav">'+ nav.join("") +'</div>');

				var $ul = $wrap.find("ul");
				var $navs = $wrap.find(".dd-nav a");
				var width = $mask.width();
				var height = $mask.height();
				var cur = 1;

				$ul.css({width: width * len, height: height});
				$wrap.find("li").css({width: width, height: height});
				$navs.first().addClass("nav-on");
				
				$ul.hammer().on("swipeleft dragleft", function(){
					cur++;
					if(cur > len){
						cur = len;
						return;
					...
					if(cur <= len){
						...
						$navs.removeClass("nav-on").eq(cur - 1).addClass("nav-on");
					}
				});

				$ul.hammer().on("swiperight dragright", function(){
					cur--;
					if(cur == 0){
						cur = 1;
						return;
				...
				$ul.hammer().on("tap doubletap", function(){
					$mask.remove();
					$wrap.remove();
				});
