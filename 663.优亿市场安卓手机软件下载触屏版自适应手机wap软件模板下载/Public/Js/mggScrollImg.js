...
function mggScrollImg(box,config){
    this.box = Zepto(box);
    this.config = Zepto.extend({},config||{});
    this.width = this.config.width||this.box.children().eq(0).width() + 10;//一次滚动的宽度
    ...
    this.auto_wait_time = this.config.auto_wait_time||30;//轮播间隔
...
Zepto.extend(mggScrollImg.prototype,{
    init : function(){
        this.bind_event();
        this.init_loop();
        this.auto_scroll();
...
/*
    ...
    go ：滚动到指定页
*/
Zepto.mggScrollImg = function(box,config){
